...
pub struct EventPoll<H: Sized> {
    ...
    port: RawFd, // The OS event port fd
}
...
struct Event<H> {
    ...
    portev_user: usize, // The portev_user ptr
...
impl<H: Sync + Send> EventPoll<H> {
    ...
    pub fn new_periodic_event(&self, handler: H, period: Duration) -> Result<EventRef, Error> {
        ...
        let tfd = match unsafe { timerfd_create(CLOCK_MONOTONIC, TFD_NONBLOCK) } {
    ...
    pub fn wait<'a>(&'a self) -> WaitResult<'a, H> {
        ...
        if unsafe { port_get(self.port, &mut event, null_mut()) } == -1 {
            return WaitResult::Error(errno_str());
        }

        let event_data = unsafe { (event.portev_user as *mut Event<H>).as_ref().unwrap() };
        ...
        if event.portev_events & POLLHUP as i32 != 0 {
            // End of file flag
    ...
    fn register_event(&self, ev: Event<H>) -> Result<EventRef, Error> {
        ...
        ev.portev_user = ev.as_mut() as *mut Event<H> as _;
        let user = ev.portev_user;
    ...
    fn insert_at(&self, index: usize, data: Box<Event<H>>) {
        ...
        if events[index].take().is_some() {
            ...
            unsafe {
                port_dissociate(self.port, PORT_SOURCE_FD, index as usize);
            };
        }
...
impl<H> EventPoll<H> {
    ...
    pub unsafe fn clear_event_by_fd(&self, index: RawFd) {
        ...
        if let Some(_) = events[index as usize].take() {
            port_dissociate(self.port, PORT_SOURCE_FD, index as _);
        }
    }
}
